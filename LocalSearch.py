...
class LocalSearch(object):
    ...
    def getTaskAssignmentsSortedByAssignedCPULoad(self, solution):
        ...
        for task in tasks:
            ...
            taskAssignment = (task, cpu, highestLoad)
        ...
        if(self.policy == 'BestImprovement'): return(taskAssignments)
        
        # Sort task assignments by the load of the assigned CPU in descending order.
        sorted_taskAssignments = sorted(taskAssignments, key=lambda taskAssignment:taskAssignment[2], reverse=True)
        return(sorted_taskAssignments)
    ...
    def exploreNeighborhood(self, solution):
        ...
        if(self.nhStrategy == 'Reassignment'):
            sortedTaskAssignments = self.getTaskAssignmentsSortedByAssignedCPULoad(solution)
                
            for taskAssignment in sortedTaskAssignments:
                task = taskAssignment[0]
                taskId = task.getId()
                
                curCPU = taskAssignment[1]
            ...
            # For Exchange neighborhood and first improvement policy, try exchanging
            # tasks two tasks, one from highly loaded CPU and the other from lowly loaded
            # CPU. It can be done by picking task1 from begin to end of sortedTaskAssignments,
            # and task2 from end to begin.
            
            sortedTaskAssignments = self.getTaskAssignmentsSortedByAssignedCPULoad(solution)
            numAssignments = len(sortedTaskAssignments)
            
            for i in xrange(0, numAssignments):             # i = 0..(numAssignments-1)
                taskAssignment1 = sortedTaskAssignments[i]
                
                task1 = taskAssignment1[0]
                taskId1 = task1.getId()
                
                curCPU1 = taskAssignment1[1]
                ...
                for j in xrange(numAssignments-1, -1, -1):  # j = (numAssignments-1)..0
                    ...
                    taskAssignment2 = sortedTaskAssignments[j]
                    
                    task2 = taskAssignment2[0]
                    taskId2 = task2.getId()
                    
                    curCPU2 = taskAssignment2[1]
