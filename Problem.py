...
class Problem(object):
    ...
    for indexX in xrange(0, nLocations):  
        ...
        if(aTime==0):
            print "timeWindow 1: ",timeWindow[min_index][0]
            #if the lower[0] timeWidow is bigger that the distance, the arrivalTime is the 
            # sum of the lower timeWidow and the tl
            if(timeWindow[min_index][0]>=min_value):
                aTime = timeWindow[min_index][0] + tl[min_index-1] 
            ...
            if(aTime<=maxTime):
                print "timeWindow 2: ",timeWindow[min_index][0]
                #if the lower[0] timeWidow is bigger that the distance, the arrivalTime is the 
                # sum of the lower timeWidow and the tl
                if(timeWindow[min_index][0]>=min_value):
                    aTime = arrivalTime[indexX-1] + timeWindow[min_index][0] + tl[min_index-1]
                ...
                vehicleId += 1
                vehi = Vehicle(vehicleId)
                print "vehi",vehi.getLocationIds()
                print "vehicles",vehicles
                #visitedLocationsDestination.append()
                # min_index = 
                # min_value = 
                # locationsDestination[positionX]        
